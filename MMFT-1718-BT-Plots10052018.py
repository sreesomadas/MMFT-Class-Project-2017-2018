...
def createRoute(con, cur):
    ...
    cur.execute(sql)          
    con.commit()
def createMeasureableRoute(con,cur):
    sql = "DROP TABLE IF EXISTS MeasureableRoute;"
    cur.execute(sql)          
    sql="CREATE TABLE MeasureableRoute ( \
            emp_no SERIAL PRIMARY KEY, \
            routeID text, \
...
def makeRoutes(con,cur):
    ...
    for i_orig in range(0,N):
        for i_dest in range(0,N):
            ...
            cur.execute(sql)    
    con.commit()
def makeMeasureableRoutes(con,cur):
    print("define a measureable route from each sensor to each other")
    sql = "SELECT * FROM BluetoothSite;"
    df_measureablesites = gpd.GeoDataFrame.from_postgis(sql,con,geom_col='geom')
    N = df_measureablesites.shape[0]
    for i_orig in range(0,N):
        for i_dest in range(0,N):
            for i_mid in range(0,N):
                if (i_orig==i_dest) or (i_orig==i_mid) or (i_dest==i_mid):  #no route from self to self
                    continue
            originSiteID = df_measureablesites.iloc[i_orig]['siteid']
            midSiteID = df_measureablesites.iloc[i_mid] ['siteid']
            destSiteID = df_measureablesites.iloc[i_dest]['siteid']
            routeID = originSiteID+">"+midSiteID+">"+destSiteID
            sql = "INSERT INTO MeasureableRoute (routeID, originSiteID, midSiteID, destSiteID ) \
            VALUES ('%s', '%s', '%s', '%s')"\
            %(routeID, originSiteID, midSiteID, destSiteID)
            print(sql)
            cur.execute(sql)    
...
importDetections()
createRoute(con,cur)
createMeasureableRoute(con,cur)
makeRoutes(con,cur)
makeMeasureableRoutes(con,cur)
