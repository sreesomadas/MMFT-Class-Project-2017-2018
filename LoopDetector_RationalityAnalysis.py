...
def importLoopHourlyFlows():
    ...
    cur.execute(sql)    
    sql = "CREATE TABLE LoopHourlyFlow ( id serial, siteIDD text, timestamp timestamp, Cosit text, LaneNumber text,     LaneDirection text, DirectionDescription text, Volume int, HGV int, TotalVolume int );"
    cur.execute(sql)    
    ...
    for fn in sorted(os.listdir(dir_detections)):  #import ALL sensor files
        ...
        for i in range(0, df_detections.shape[0]):   #here we use Python's DateTime library to store times properly
            ...
            sql = "INSERT INTO LoopHourlyFlow (siteIDD, Cosit, timestamp, LaneNumber, LaneDirection,            DirectionDescription,Volume, HGV, TotalVolume )             VALUES ('%s', '%s', '%s', '%s', '%s', '%s', %s, %s, %s );            "%(siteID, df_detections.iloc[i]['Cosit'], dt, df_detections.iloc[i]['LaneNumber'], df_detections.iloc[i]['LaneDirection'],            df_detections.iloc[i]['DirectionDescription'], df_detections.iloc[i]['Volume'],             df_detections.iloc[i]['PmlHGV'], df_detections.iloc[i]['TotalVolume'] )
            cur.execute(sql)
    con.commit() 
def ImportLoopDailyFlows1():
    ...
    cur.execute(sql)    
    sql = "CREATE TABLE LoopDailyFlow1 (idd serial,     DailyVolume int, DailyHGVs int, DailyTotalVolume int);"
    cur.execute(sql)    
    sql = "INSERT INTO LoopDailyFlow1 (DailyVolume, DailyHGVs, DailyTotalVolume)    SELECT SUM(Volume), SUM(HGV), SUM(TotalVolume)    FROM LoopHourlyFlow GROUP BY siteIDD ORDER BY siteIDD;"
    cur.execute(sql)  
con.commit() 
def ImportLoopDailyFlows2():
    ...
    cur.execute(sql) 
    sql = "INSERT INTO LoopDailyFlow2 (siteIDD)     SELECT DISTINCT siteIDD FROM LoopHourlyFlow     ORDER BY siteIDD;"
    cur.execute(sql)    
con.commit() 
def ImportLoopDailyFlows():
    ...
    cur.execute(sql)
    sql = "CREATE TABLE loopdailyflow AS SELECT * FROM LoopDailyFlow2     JOIN LoopDailyFlow1 ON LoopDailyFlow2.ida=LoopDailyFlow1.idd;"
    cur.execute(sql)     
con.commit() 
def FlowSite1():
