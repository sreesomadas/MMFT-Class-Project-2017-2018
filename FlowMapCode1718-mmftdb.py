...
con = psycopg2.connect(database='mmft', user='root')
cur = con.cursor()
...
def importLoopSites():
    ...
    cur.execute(sql)
    con.commit()    
    fn_sites = "/headless/MMft_Class_Project-1718/LoopDetectorData/loop_detector_sites_new1.csv"
...
def importDetections():
    ...
    cur.execute(sql)    
    sql = "CREATE TABLE Detection ( id serial, siteID text, Cosit text, timestamp timestamp, volume text );"
    cur.execute(sql)    
    dir_detections = "/headless/MMft_Class_Project-1718/LoopDetectorData/LoopData/"
    for fn in sorted(os.listdir(dir_detections)):  #import ALL sensor files
        ...
        for i in range(0, df_detections.shape[0]):   #here we use Python's DateTime library to store times properly
            ...
            sql = "INSERT INTO Detection (siteID, timestamp, Cosit) VALUES ('%s', '%s', '%s');"%(siteID, dt, df_detections.iloc[i]['Volume'])
            cur.execute(sql)
    con.commit()    
def plotRoads():   
...
def routeCounts(con,cur):   #count number of matching Bluetooth detections between origins and destinations
    ...
    for i in range(0, df_route.shape[0]):    #each route
        ...
        #Cosit matching
        sql = "SELECT d.siteID AS dSiteID,  d.Cosit as dCosit, d.timestamp as dtimestamp  ,   o.siteID AS oSiteID,  o.Cosit as oCosit, o.timestamp as otimestamp    FROM Detection AS d, Detection AS o  WHERE d.timestamp>o.timestamp  AND o.Cosit=d.Cosit  AND o.siteID='%s' AND d.siteID='%s'"%(oSiteID, dSiteID)
        print(sql)
