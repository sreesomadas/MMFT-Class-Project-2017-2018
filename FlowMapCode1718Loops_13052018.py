...
con = psycopg2.connect(database='loop', user='root')
cur = con.cursor()
...
def importLoopSites():
    ...
    cur.execute(sql)
    con.commit()    
    fn_sites = "/headless/data/dcc/loop/loopDetectorSites.csv"
...
def importLoopHourlyFlows():
    ...
    cur.execute(sql)    
    sql = "CREATE TABLE LoopHourlyFlow ( id serial, siteID text, timestamp timestamp, Cosit text, LaneNumber text, \
    LaneDirection text, DirectionDescription text, Volume int, HGV int, TotalVolume int );"
    cur.execute(sql)    
    dir_detections = "/headless/data/dcc/loop/"
    for fn in sorted(os.listdir(dir_detections)):  #import ALL sensor files
        ...
        for i in range(0, df_detections.shape[0]):   #here we use Python's DateTime library to store times properly
            ...
            sql = "INSERT INTO LoopHourlyFlow (siteID, Cosit, timestamp, LaneNumber, LaneDirection,\
...
def ImportLoopDailyFlows1():
    ...
    FROM LoopHourlyFlow GROUP BY siteID, LaneDirection ORDER BY siteID, \
    LaneDirection;"
    cur.execute(sql)  
con.commit() 
def ImportLoopDailyFlows2():
    ...
    cur.execute(sql)   
    sql = "CREATE TABLE LoopDailyFlow2 (id serial, siteID text,\
    LaneDirection text);"
    cur.execute(sql) 
    sql = "INSERT INTO LoopDailyFlow2 (siteID, LaneDirection) \
    SELECT DISTINCT siteID, LaneDirection FROM LoopHourlyFlow \
    ORDER BY siteID, LaneDirection";
    cur.execute(sql)    
con.commit() 
def ImportLoopDailyFlows():
    ...
    JOIN LoopDailyFlow1 ON LoopDailyFlow2.id=LoopDailyFlow1.idd;"
    cur.execute(sql)     
con.commit() 
def plotRoads():   
    ...
    for index, row in df_roads.iterrows():
        ...
        plot(xs, ys, color)
    xlabel('Longitude'); ylabel('Latitude'); title("Chesterfield's road network and Loop detector sites")
def plotLoopSites():    
    sql = "SELECT siteID, geom FROM LoopSite;"
...
def makeRoutes(con,cur):
    print("define a route between each loop")
    sql = "SELECT * FROM LoopSite;"
    df_sites = gpd.GeoDataFrame.from_postgis(sql,con,geom_col='geom')
    N = df_sites.shape[0]
...
def routeLinks(con,cur): 
    ...
    sql = "SELECT routeID, ST_X(orig.geom) AS ox, ST_Y(orig.geom) AS oy, ST_X(dest.geom) AS dx, ST_Y(dest.geom) AS dy  FROM Route, LoopSite AS orig, LoopSite AS dest WHERE originSiteID=orig.siteID AND destSiteID=dest.siteID;"  #join useful data
    ...
    for i in range(0, N):
        ...
        for i in range(0,df_route.shape[0]):
            ...
            cur.execute(sql)
        con.commit()
def routeCounts(con,cur):   #count number of matching Loop detections between origins and destinations
    ...
    for i in range(0, df_route.shape[0]):    #each route
        ...
        #Cosit matching
        sql = "SELECT d.siteID AS dSiteID,  d.Cosit as dCosit, d.timestamp as dtimestamp  ,   o.siteID AS oSiteID,  o.Cosit as oCosit, o.timestamp as otimestamp    FROM LoopHourlyFlow AS d, LoopHourlyFlow AS o  WHERE d.timestamp>o.timestamp  AND o.Cosit=d.Cosit  AND o.siteID='%s' AND d.siteID='%s'"%(oSiteID, dSiteID)
        print(sql)
...
def plotFlows(con,cur):     
    dt_start  = datetime.datetime.strptime('2017-02-14_00:00:00' , "%Y-%m-%d_%H:%M:%S" )
    dt_end    = datetime.datetime.strptime('2017-02-14_23:00:00' , "%Y-%m-%d_%H:%M:%S" )
