...
def cleanedDetections(con,cur):
    ...
    cur.execute(sql)
    sql="DELETE FROM Detection1 d1 USING Detection1 d2 WHERE \
...
s=folium.Map(location=[53.235048, -1.421629], zoom_start=13)
locations=[[53.2309453,-1.4219618], [53.2308295,-1.421946], [53.2307102,-1.4220285]]
folium.PolyLine(locations, color='red', \
                    weight=5).add_to(s)
s.save("chesterfield55.html")  
# Try plotting outside of def
df_links22=pd.read_csv('df_links.csv')
for i in range(0,df_links22.shape[0]): 
    link = df_links22[i]
    lons = link['the_geom'].coords.xy[0] #coordinates in latlon
    lats = link['the_geom'].coords.xy[1]
    #gid = int(link.gid) 
