...
def cleanedDetections(con,cur):
    sql="DROP TABLE IF EXISTS Detection1;"
    cur.execute(sql)
    sql = "CREATE TABLE Detection1 AS SELECT * FROM Detection;"
    cur.execute(sql)
    sql="DELETE FROM Detection1 d1 USING Detection1 d2 WHERE d2.mac=d1.mac\
            AND d2.siteid=d1.siteid AND \
            d2.timestamp-d1.timestamp<interval '10 second' AND d2.id<d1.id;"
    cur.execute(sql)          
    con.commit() 
cleanedDetections(con,cur)
def cleanedDetections(con,cur):
    sql="DROP TABLE IF EXISTS CleanedDetection1;"
    cur.execute(sql)
    sql = "CREATE TABLE CleanedDetection1 AS SELECT FROM Detection d1 WHERE \
            EXISTS (SELECT id from detection d2 WHERE d2.timestamp>d1.timestamp \
            AND d2.timestamp<d1.timestamp+interval '30 sec' AND \
            d2.siteid=d1.siteid and d2.mac=d1.mac);"
    cur.execute(sql)          
    con.commit() 
cleanedDetections(con,cur)
...
def plotSelectedODRoutes():
    ...
    sql="SELECT * FROM OtoM WHERE odrouteid='MAC000010101>MAC000010104>MAC000010119' \
        AND odrouteid='MAC000010101>MAC000010118>MAC000010119';"
    df_links = gpd.GeoDataFrame.from_postgis(sql,con,geom_col='the_geom') 
    """for i in range(0,df_links.shape[0]): 
        link = df_links.iloc[i]
        lons = link['the_geom'].coords.xy[0] #coordinates in latlon
    ...
    folium.PolyLine(locations=locations, color='red', \
                    weight=5).add_to(s)
