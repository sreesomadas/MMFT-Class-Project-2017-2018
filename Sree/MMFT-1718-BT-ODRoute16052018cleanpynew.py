...
abc = psql.read_sql("SELECT * FROM Detection", con)

for id, row in abc.iterrows():
    #print (id, row['mac'])
...
for id, row in abc.iterrows():
    ...
    if id >= 15:
        break
#con.commit()
sql = "CREATE TABLE DetectionNew ( id serial, siteID text, mac text, \
timestamp timestamp );"
#cur.execute(sql) 
#cde = psql.read_sql_query("SELECT * FROM Detection", con)
#print (cde)
print (abc)
#import sql
#abc.to_sql("detection3",con, if_exists='replace', flavor='postgresql')
test = abc.to_csv('detectionnew.csv', sep='\t')
test1 = 'detectionnew.csv'
print (test)
df_sites = pd.read_csv(test, header=1) 
sql = "INSERT INTO DetectionNew ( id serial, siteID text, mac text, \
timestamp timestamp );"'''
...
def ODrouteLinks(con,cur): 
    ...
    df_omd = pd.read_sql_query(sql,con)
    N = df_omd.shape[:]
    for i in range(0, N):
        OMDrouteID = df_omd.loc[i]['ODrouteID']
        o_easting  = df_omd['ox'].iloc[i] #link-broken table is stored as latlon so convert on the fly
        o_northing = df_omd['oy'].iloc[i]
        m_easting  = df_omd['mx'].iloc[i]
        m_northing = df_omd['my'].iloc[i]
        d_easting  = df_omd['dx'].iloc[i]
        d_northing = df_omd['dy'].iloc[i]
        ...
        for i in range(0,df_route.shape[0]):
            ...
            sql = "INSERT INTO ODroutelink (ODrouteID, link_gid, origin_length, \
            mid_length, dest_length, total_length ) \
            VALUES ('%s', '%s','%s','%s','%s');"%(OMDrouteID, rc['gid'], \
