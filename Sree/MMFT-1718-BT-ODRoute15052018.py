...
def DetectionClean():
    ...
    timestamp timestamp);"
    cur.execute(sql)
    sql = "SELECT Detection.siteid,Detection.mac,Detection.timestamp FROM \
    ( SELECT siteid,mac,timestamp, RN = ROW_NUMBER() \
    OVER(ORDER BY siteid) \
    FROM Detection) Detection WHERE Detection.rn=1"
  #sql = "INSERT INTO DetectionClean SELECT * FROM Detection ORDER BY \
    #min(timestamp) DISTINCT ON\
    #(siteid,mac)Detection. * FROM Detection
    print (sql)
    cur.execute(sql)
    con.commit()    
def plotRoads():   
...
def ODrouteLinks(con,cur): 
    ...
    sql = "SELECT OMDrouteID, ST_X(orig.geom) AS ox, ST_Y(orig.geom) \
